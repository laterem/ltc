...
class ConvertBase(LTCFunction):
    ...
    def call(self):
        ...
        num = int(integral + fractional, base_from) * base_from ** -len(
            fractional
        )
...
class RandomFloat10(LTCFunction):
    ...
    def call(self):
        ...
        return (
            randint(
                int(float(self.args[0]) * percision),
                int(float(self.args[1]) * percision),
            )
            / percision
        )
...
class IsMetricEqual(LTCFunction):
    ...
    def call(self):
        return (
            self.args[0].endswith(self.args[2])
            and float(self.args[0].strip(self.args[2])) == self.args[1]
        )
...
class IsEqual(LTCFunction):
    ...
    def call(self):
        ...
        try:
            return normalize(self.args[0]) == normalize(self.args[1])
        except ValueError:
            return False
...
class IsEqualText(LTCFunction):
    ...
    def call(self):
        return self.args[0].strip().lower().replace("ё", "е") == str(
            self.args[1]
